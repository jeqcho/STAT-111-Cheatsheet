...
The steps you can carry out to find the MLE of $\theta$ given the data $\vec y$:
\begin{enumerate}
...
Given a constant $c$, and r.v. $Y_n$, we have $Y_n\xrightarrow{p}c$ implies $Y_n\xrightarrow{d}c$.
...
Let $Y_1,\dots,Y_n$ be i.i.d. random variables with finite first moments, i.e. $\E (|Y_1|)<\infty$. Then, SLLN says $\bar{Y}\xrightarrow{a} \E (Y_1)$ and WLLN says $\bar{Y}\xrightarrow{p} \E (Y_1$). Similar to CLT we have for general moments
$$
...
\begin{enumerate}
    ...
    \item P-value is the proporition of times that $t$ was at least as extreme as $t_0$.
    $$
